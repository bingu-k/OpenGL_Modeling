...
int main_process()
{
    ...
    std::unique_ptr<Program>    objectProgram = Program::Create("./shader/normalMap.vert", "./shader/deffered.frag");
    std::unique_ptr<Program>    FBProgram = Program::Create("./shader/framebuffer.vert", "./shader/deffered_light.frag");
    ...
    // FrameBuffer
    std::unique_ptr<FrameBuffer>    framebuffer = FrameBuffer::Create({{Texture::Create(width, height, GL_RGBA16F, GL_FLOAT)},
    ...
    // Lighting Set
    FBProgram->Use();
    FBProgram->setUniform(false, "lightswitch[0]");
    FBProgram->setUniform(false, "lightswitch[1]");
    FBProgram->setUniform(false, "lightswitch[2]");
    FBProgram->setUniform(0, "gPosition");
    FBProgram->setUniform(1, "gNormal");
    FBProgram->setUniform(2, "gAlbedoSpec");
    ...
    Plight->setUniform(FBProgram.get());
    ...
    Dlight->setUniform(FBProgram.get());
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        FBProgram->Use();
        for (int i = 0; i < framebuffer->GetColorAttachementCount(); ++i)
            framebuffer->GetColorAttachment(i)->bind(i);
        FBProgram->setUniform(camera->getPosition(), "viewPos");
        glm::mat4   transform = MathOP::scale(glm::mat4(1.0f), glm::vec3(2.0f, 2.0f, 2.0f));
        FBProgram->setUniform(transform, "transform");
        FBProgram->setUniform(64.0f, "shininess");

        // Spot Light(유동적)
        Slight->setPos(camera->getPosition());
        Slight->setDir(camera->getFront());
        Slight->setUniform(FBProgram.get());
        ...
        // Point Lighting Box
        lightProgram->Use();
        model = MathOP::translate(glm::mat4(1.0f), lightPos);
        model = MathOP::scale(model, glm::vec3(0.125f));
        lightProgram->setUniform(view * model, "transform");
        lightProgram->setUniform(glm::vec4(0.78f), "lightColor");
        box->Draw();
